...
class Crops extends CI_Controller{
	...
	public function addPlantCrop(){
		...
		$this->form_validation->set_rules('plantManufacturer', 'plantManufacturer', 'required');
		$this->form_validation->set_rules('dateStart', 'Start Date', 'required');
		$this->form_validation->set_rules('testing', 'Testing', 'required');
		$this->form_validation->set_rules('location', 'Location','required');
		//run validation
		if($this->form_validation->run()){
			...
			//load view for validation
			$this->load->view('templates/header3');
			$this->load->view('templates/validate_Plantcrop', $data);
		}else{
			$this->load->view('templates/header3');
			$this->load->view('errors/crop_error');
	...
	public function validatePlantCrop(){
		$data = array(
				'plantName' => $this->input->post('plantName'),
				'plantAmount' => $this->input->post('plantAmount'),
				'plantManufacturer' => $this->input->post('plantManufacturer'),
			...
			$query = $this->db->insert('plantcrops', $data);
			$this->load->view('templates/header3');
			$this->load->view('success/addSuccess');
	}
	...
	public function updatePlantGrow(){
		$this->load->model('Crop');
		$this->load->library('form_validation');
		//set validation rules
		$this->form_validation->set_rules('plantNumber', 'Plant Number', 'required' );
		$this->form_validation->set_rules('dateEnd', 'Harvest Date', 'required');
		if($this->form_validation->run()){
			$W = $this->input->post('weight');
			$YW = $this->input->post('yWeight');
		if($W === "oz"){
			...
			$data1 = $this->input->post("plantNumber");
			$data  = array(
				'plantNumber' => $this->input->post('plantNumber'),
				'plantName' => $this->input->post('plantName'),
				'dateEnd' => $this->input->post('dateEnd'),
				'yieldWeight' => $conAmount,
				'cropRating' => $this->input->post('cropRating')
			...
			$this->load->view('templates/header3');
			$this->load->view('success/updatePlantSuccess', $data);
		}else{
			$this->load->view('templates/header3');
			$this->load->view('errors/crop_error');
		}
	...
	public function validatePlantGrow(){
		$this->load->model('Crop');
		$data1 = $this->input->post("plantNumber");
			$data  = array(
				'dateEnd' => $this->input->post('dateEnd'),
		...
		$this->Crop->updatePlantEnd($data1, $data);
		$this->load->view('templates/header3');
		$this->load->view('success/addSuccess');
	}
